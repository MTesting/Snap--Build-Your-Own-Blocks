...
IDE_Morph.prototype.publishModuleBrowser = function () {
    this.stage.globalBlocks.length > 0 ? new ModuleExportDialogMorph(this, 'publish').popUp() : this.showMessage("No custom blocks", 2);
};
...
ModuleImportDialogMorph.prototype.buildContents = function () {
    ...
    this.myModuleCheckBox = new ToggleMorph(
        ...
        function () {
            ...
            myself.descriptionNotesField.setContents(new TextMorph('Description...'));

            myself.informationNotesField.setContents( new TextMorph('Information...'));
    ...
    this.nameField.contents().childChanged = function (child) {
        if (child instanceof StringMorph) {
            ...
            myself.moduleList = [];
            SnapCloud.getModules(
                function (modJSON) {
                    myself.moduleList = JSON.parse(modJSON);
                    myself.loadModuleList(myself.moduleList);
                    myself.fixListFieldItemColors();
                    myself.fixLayout();
                },
    ...
    this.modulesListField = new ListMorph([]);
    ...
    this.descriptionNotesField.setContents(new TextMorph('Description...'));
    this.descriptionNotesField.setWidth(200);

    this.body.add(this.descriptionNotesField);

    this.informationNotesField = new ScrollFrameMorph();
    this.informationNotesField.fixLayout = nop;

    this.informationNotesField.edge = InputFieldMorph.prototype.edge;
    this.informationNotesField.fontSize = InputFieldMorph.prototype.fontSize;
    this.informationNotesField.typeInPadding = InputFieldMorph.prototype.typeInPadding;
    this.informationNotesField.contrast = InputFieldMorph.prototype.contrast;
    this.informationNotesField.drawNew = InputFieldMorph.prototype.drawNew;
    this.informationNotesField.drawRectBorder = InputFieldMorph.prototype.drawRectBorder;
    ...
    this.informationNotesField.setContents(new TextMorph('Information...'));
...
ModuleImportDialogMorph.prototype.fixListFieldItemColors = function () {
    ...
    // to take effect
    var myself = this;
    this.modulesListField.contents.children[0].alpha = 0;
    this.modulesListField.contents.children[0].children.forEach(function (item) {
...
ModuleImportDialogMorph.prototype.loadBlockList = function () {
    console.log("Loading block list!");
    if (this.selection) {
        var myself = this;
        this.blocks = [];

        this.descriptionNotesField.setContents(new TextMorph(this.selection.description));

        this.informationNotesField.setContents(new TextMorph("Name: " + this.selection.name + "\nAuthor: " + this.selection.user + "\nLast Update: " + this.selection.updated));

        console.log('Loading module contents');
        SnapCloud.getModuleContents(
            function(contents) {
                myself.blocks = myself.ide.serializer.loadBlocks(contents, myself.ide.stage);

                myself.buildCanvas();
                myself.fixLayout();
            },
            myself.ide.cloudError(),
            myself.selection.user,
            myself.selection.name
        );
    }
}
...
ModuleImportDialogMorph.prototype.loadModuleList = function (modules) {
    ...
    if (myself.myModuleCheckBox.state) {
        modules = modules.filter(function(element) {
            return (element['user'] === SnapCloud.username);
        });
    }

    var fname = modules.filter(function(element) {
        return (element["name"].toUpperCase().indexOf(myself.nameField.contents().text.text.toUpperCase()) > -1);
    });

    var modules = fname.concat(modules.filter(function(element) {
        return (element["description"].toUpperCase().indexOf(myself.nameField.contents().text.text.toUpperCase()) > -1 &&
            fname.every(function(felement) {
                return (element["name"] !== felement["name"] || element["user"] !== felement["user"]);
            })
        );
    }));

    if (modules.length === 0) {
        modules = [{"name": "No modules found"}];
    }

    this.modulesListField = new ListMorph(modules, function (element) {return element['name'];});
    this.modulesListField.fixLayout = nop;
    this.modulesListField.edge = InputFieldMorph.prototype.edge;
    this.modulesListField.fontSize = InputFieldMorph.prototype.fontSize;
    this.modulesListField.typeInPadding = InputFieldMorph.prototype.typeInPadding;
    ...
    this.modulesListField.action = function (element) {
        ...
        myself.selection = element;
        myself.loadBlockList();
...
ModuleImportDialogMorph.prototype.rawImportModule = function () {
    var myself = this;
...
ModuleImportDialogMorph.prototype.downloadModule = function () {
    ...
    if (this.selection) {
        SnapCloud.getModuleContents(
            function (contents) {
                myself.ide.saveXMLAs(contents, myself.selection.name);
                myself.destroy();
            },
            myself.ide.cloudError(),
            myself.selection.user,
...
ModuleImportDialogMorph.prototype.updateModule = function () {
    if (this.modulesListField.selected) {
        if (this.selection && this.selection.user === SnapCloud.username) {
...
function ModuleExportDialogMorph(ide, label, name, blocks) {
    this.init(ide, label, name, blocks);
}

ModuleExportDialogMorph.prototype.init = function (ide, task, name, blocks) {
    var myself = this;
    // additional properties:
    this.ide = ide;
    this.task = task || 'publish';
    this.module = name || '';
    this.blocks = blocks || [];
    this.selectedBlocks = [];
...
ModuleExportDialogMorph.prototype.buildContents = function () {
    ...
    this.moduleNameField = this.task === 'publish'? new InputFieldMorph('Module name') : new TextMorph(this.module,null,null,true);
    ...
    this.task === 'publish'? this.addButton('publishModule', 'Publish') : this.addButton('updateModule', 'Update');
    this.action = 'exportModule';

    this.addButton('cancel', 'Cancel')
    if (this.task === 'update'){
        this.selectedBlocks = this.blocks;
        this.blocks = this.blocks.concat(this.ide.stage.globalBlocks.filter(function (element) {
             return myself.blocks.every(function (belement) {
                 return (belement.blockSpec() !== element.blockSpec());
             });
         }));
    } else {
        this.selectedBlocks = [];
        ...
        myself.blocks.forEach(function (definition) {
            if (definition.category === category) {
                ...
                checkBox.mouseEnter = function () {
                    checkBox.bubbleHelp(definition.comment ? definition.comment.text() : "No description");
                }
...
ModuleExportDialogMorph.prototype.requestModule = function (user, module){
    ...
    SnapCloud.getModuleContents(
        function (moduleContents) {
            ...
            if (myself.blocks.length === 0) {
                myself.ide.showMessage("No custom blocks", 2);
                myself.destroy();
            }
